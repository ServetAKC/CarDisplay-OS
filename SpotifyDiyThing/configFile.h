    ...
    if (configFile) {
      ...
      if (!error) {
        ...
        if (json.containsKey(REFRESH_TOKEN_LABEL)) {
          strlcpy(refreshToken, json[REFRESH_TOKEN_LABEL] | "", 400);
        }

        if (json.containsKey(CLIENT_ID_LABEL) && json.containsKey(CLIENT_SECRET_LABEL)) {
          strlcpy(clientId, json[CLIENT_ID_LABEL] | "", 50);
          strlcpy(clientSecret, json[CLIENT_SECRET_LABEL] | "", 50);
          if (clientId[0] == '\0' || clientSecret[0] == '\0') {
            Serial.println("Config has an empty client ID or Secret");
            return false;
          }
        } else {
